...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
    	...
    	return Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent()).collect(Collectors.groupingBy(Student::getFirstName, Collectors.counting()))
    			.entrySet().parallelStream().max((k1,k2) ->  (k1.getValue().intValue() - k2.getValue().intValue())).get().getKey();
    			
//    			.sorted((k1,k2) ->  (k2.getValue().intValue() - k1.getValue().intValue()))
